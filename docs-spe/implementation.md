...
Delivery is opt-in per user, per notification type. Preferences are owned by the notification context itself: a `PreferencesService` stores them through a `PreferencesRepository` (one record per home/user pair, persisted in a dedicated MongoDB collection), and users without a stored record default to receiving every type. Before sending, `NotificationService` queries `UserPreferencesPort.getEnabledUsernamesForType` — implemented by the `PreferencesService` — so a notification only reaches users who are subscribed to that category. To resolve the candidate recipients, the `PreferencesService` fetches the home's members and their roles from the user context through the `HomeUsersPort` outbound port, whose `UserContextAdapter` translates the user context's model into the notification context's own `HomeUser` shape. A single incoming event may therefore produce zero, one, or many `Notification` domain objects, one per opted-in user. Each is stored in the database and delivered individually using Socket.IO's `user-${username}` room, so each notification reaches exactly its intended recipient regardless of how many connections that user has open.
